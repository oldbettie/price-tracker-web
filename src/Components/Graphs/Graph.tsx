                    ...
                    <>
                        <div className={styles.productInfo}>
                            <h1 className={styles.productName}>
                                <a href={productInfo[0].url}>{productInfo[0].name}</a>
                            </h1>
                            <p className={styles.productNormalPrice}>
                                The normal price is - ${productInfo[lastIndex].fullPrice}
                            </p>
                            <p className={styles.productCurrentPrice}>
                                The current price is - ${productInfo[lastIndex].currentPrice}
                            </p>
                            {productInfo[lastIndex].discountAmount && (
                                <p className={styles.productDiscount}>
                                    The current discount is - ${productInfo[lastIndex].discountAmount} for a total{" "}
                                    {productInfo[lastIndex].discountPercentage}% off
                                </p>
                            )}
                            <p className={styles.productDate}>
                                This was last checked on - {productInfo[lastIndex].date}
                            </p>
                            <p className={styles.productAverage}>
                                The average price in the last {inLastDays.inLastDays} days is - $
                                {inLastDays.averagePrice}
                            </p>
                            <h3>Checkout the price of this product over the last 2 weeks.</h3>
                            <p>
                                Currently the price is checked 3 times per day future iterations will check regularly
                                for price changes and only track when that happens.
                            </p>
                        </div>
